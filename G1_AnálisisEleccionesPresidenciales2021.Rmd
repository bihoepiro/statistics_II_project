...
### Caso Estudio 1:
Ciertos medios de comunicación afirman que menos del 40% de los votos en las mesas de la segunda vuelta en Lima fueron para Perú Libre.
Si la afirmación es correcta no se procederá a reportar un caso de desinformación, caso contrario se reportará. 
Teniendo como muestra 1000 mesas.
Usando un nivel de significancia del 5% ¿Qué decisión se debe tomar?

```{r}
M_C_L <- DatosAbiertos %>% filter(departamento == "LIMA")
muestras_aleatorias <- M_C_L %>% 
  sample_n(1000, replace = FALSE)
muestras_aleatorias
```

```{r}
total <- sum(muestras_aleatorias$total_votantes, na.rm = TRUE)
votos_pl1 <- sum(muestras_aleatorias$votos_pl, na.rm = TRUE)

pi= votos_pl1/total
```

Hipótesis:
$$  H_{o}:\pi_\text{votos_pl} \ge 0.40 $$
$$ H_{a}:\pi_\text{votos_pl} < 0.40\\  $$
...
$$ Z_{o}=\frac{p-\pi_{o}}{\sqrt{\frac{\pi_{o}(1-\pi_{o})}{n}}} $$

```{r}
pi
PIo<- 0.40    # El valo de PI, que está en Ho.
Zo<-(pi - PIo )/sqrt(PIo*(1-PIo)/total)

print(paste("el estadístico de prueba  es",Zo))
```
Región crítica:

$$\text{Dado que es una prueba de cola izquierda,  la región sombreada estará en el lado izquierdo de la gráfica y su área es } \alpha$$ 

Valor crítico: 
es el número que está en el borde de la región sombreada y siempre se calcula con el área total que hay hacia su izquierda.

```{r}
alfa<-0.05
VC<-qnorm(alfa,0,1) 
print(paste("el valor crítico es", VC)) 
```


Decisión:

$$ \text{Como el Zo está en la región sombreada, se rechaza Ho. } $$

Conclusión:

$$ \text{Se tendrá que reportar casos de desinformación en todos los medios de comunicación que afirmen que los votos por Perú Libre fueron menor a 40% en las mesas de Lima en la segunda vuelta.} $$
### Caso Estudio 2:
### Caso Estudio 3:
### Caso Estudio 4:
...
### Caso Estudio:
Se plantea un experimento con la finalidad de comparar las preferencias de la población por ideología en las candidaturas.
Variable de entrada: votos por acta con la opción de elegir entre distintas candidaturas ligadas a una ideología
Factor: Candidatura política
Tratamiento: cada tipo de ideología 
Unidad experimental: personas hábiles electorales 2021
Variable respuesta: comprensión en la dinámica política en el Perú
Error experimental:

```{r}
CIN=sample(DatosAbiertos$primera_vuelta_accion_popular,size=600,replace=T)
Data_p2<- data.frame(
CIN=sample(DatosAbiertos$primera_vuelta_accion_popular,size=600,replace=T),
DN=sample(DatosAbiertos$primera_vuelta_app,600,T)+sample(DatosAbiertos$primera_vuelta_fuerza_popular,600,T)+sample(DatosAbiertos$primera_vuelta_renacimiento,600,T)+sample(DatosAbiertos$primera_vuelta_renovacion_popular,600,T),
DG=sample(DatosAbiertos$primera_vuelta_avanza_pais,600,T),
IN=sample(DatosAbiertos$primera_vuelta_democracia_directa,600,T)+sample(DatosAbiertos$primera_vuelta_frente_amplio,600,T)+sample(DatosAbiertos$primera_vuelta_jpp,600,T)+sample(DatosAbiertos$primera_vuelta_partido_nacionalista,600,T)+sample(DatosAbiertos$primera_vuelta_peru_libre,600,T)+sample(DatosAbiertos$primera_vuelta_upp,600,T),
CDG=sample(DatosAbiertos$primera_vuelta_partido_morado,600,T)+sample(DatosAbiertos$primera_vuelta_ppc,600,T)+sample(DatosAbiertos$primera_vuelta_pps,600,T)+sample(DatosAbiertos$primera_vuelta_victoria_nacional,600,T)+sample(DatosAbiertos$primera_vuelta_somos,600,T),
CDN=sample(DatosAbiertos$primera_vuelta_podemos_peru,600,T)
)
head(Data_p2,10)
```
```{r}
boxplot(Data_p2)
```
Se observa en el gráfico que la izquierda nacionalista como ideología tiene mayor recibimiento en la población peruana.

```{r}
SDF3<-stack(Data_p2)
Anova <- aov(values~ind, data=SDF3) 
Anova
summary(Anova) 
```
...
|--------------|------------|--------------|-------------|----------|---------|
| TRATAMIENTOS | k-1=5      | SCTr=1093184 | CMTr=218637 | Fo=569.6 | P=2e-16 |
| ERROR        | N-k=3694   | SCE=1379457  | CME=384     |          |         |
| TOTAL        | N-1=3699   | SCT=2472641  |             |          |         |

$$P_{valor}= P(F_{(k-1,N-k)}\>Fo)$$

```{r}
pf(569.6,5,3699,lower.tail = F)
```
$Ho: \mu1= \mu2 = \mu3$

$Ha: \text{Existe algun}\ \mu\ \text{distinto de otros.}$

Notar que el p valor se obtiene asI:

$$Pvalor= P(F(5,3699) > 65983)=  2e-16< \alpha = 0.05$$

$$==> Rechazo\ Ho ==> Tomo\ Ha: \text{Existe alguna media distinta de otras.}$$


**DETERMINANDO CUAL ES EL PROMEDIO DISTINTO:**

## Test de Tukey para comparaciones múltiples

Para realizar un ***test de Tukey HSD*** (Honest Significant Differences)

```{r echo=TRUE}
TukeyHSD(Anova, conf.level=0.95) 
# Anova es la tabla de ANOVA que se halló arriba. Con confianza 0.95 
```
...
+37.5717 \< mu2 - mu1 \< + 38.1804 ==\> (-) \< mu2 - mu 1 \< (+) ==\> mu2 \> mu1

+3.3770 \< mu3 - mu1 \< + 3.9857 ==\> (+) \< mu3 - mu1 \< (+) ==\> mu3\> mu1

+36.6781 \< mu4 - mu1 \< + 36.9824 ==\> (+)\< mu3 - mu2 \< (+) ==\> mu4 \> mu1

+3.9514 \< mu5 - mu1 \< + 4.5601 ==\> (-) \< mu2 - mu 1 \< (+) ==\>  mu5 \> mu1

-6.5385 \< mu6 - mu1 \< - 5.9298 ==\> (+) \< mu3 - mu1 \< (+) ==\> mu6\< mu1

-34.4990 \< mu3 - mu2 \< + -33.8904 ==\> (+)\< mu3 - mu2 \< (+) ==\> mu3 \< mu2

-1.5022 \< mu4 - mu2 \< -0.8936 ==\> (-) \< mu2 - mu 1 \< (+) ==\>  mu4 \< mu2

-33.9246 \< mu5 - mu2 \< -33.3159 ==\> (+) \< mu3 - mu1 \< (+) ==\> mu5\< mu2

-44.4146 \< mu6 - mu2 \< -43.8059 ==\> (+)\< mu3 - mu2 \< (+) ==\> mu6 \< mu2

+32.6924 \< mu4 - mu3 \<+ 33.3011 ==\> (-) \< mu2 - mu 1 \< (+) ==\> mu4 \> mu3

+0.2701 \< mu5 - mu3 \< + 0.8787 ==\> (+) \< mu3 - mu1 \< (+) ==\> mu5\> mu3

-10.2198 \< mu6 - mu3 \< -9.6112 ==\> (+)\< mu3 - mu2 \< (+) ==\> mu6 \< mu3

-32.7267 \< mu5 - mu4 \<-32.1180 ==\> (-) \< mu2 - mu 1 \< (+) ==\> mu5 \< mu4

-43.2166 \< mu6 - mu4 \< -42.6080 ==\> (+) \< mu3 - mu1 \< (+) ==\> mu6\< mu4

-10.7943 \< mu6 - mu5 \< - 10.1856 ==\> (+)\< mu3 - mu2 \< (+) ==\> mu6 \< mu5


Conclusión:

mu6<mu1<mu3<mu5<mu4<mu2
Se concluye que ninguno de los mu son iguales, 

p adj= P valor ajustado ==\> como pvalor= 0.0000 \< alfa=0.05 ==\> rechazo Ho:  mu2 \> mu1

p adj=P valor ajustado ==\> Como p valor = 0.0817 \> alfa = 0.05 ==\> Acepto Ho: mu3\> mu1

p adj= P valor ajustado ==\> como p valor = 0.0000 \< alfa = 0.05 ==\> Rechazo Ho: mu4 \> mu1

p adj= P valor ajustado ==\> como pvalor= 0.0326 \< alfa=0.05 ==\> Rechazo Ho: mu5 \> mu1

p adj=P valor ajustado ==\> Como p valor = 0.0000 \< alfa = 0.05 ==\> Rechazo Ho: mu6\< mu1

p adj= P valor ajustado ==\> como p valor = 0.0000 \< alfa = 0.05 ==\> Rechazo Ho: mu3 \< mu2

p adj= P valor ajustado ==\> como pvalor= 0.1103 \> alfa=0.05 ==\> acepto Ho: mu4 \< mu2

p adj=P valor ajustado ==\> Como p valor = 0.0000 \< alfa = 0.05 ==\> Rechazo Ho: mu5\< mu2

p adj= P valor ajustado ==\> como p valor = 0.0000 \< alfa = 0.05 ==\> Rechazo Ho: mu6 \< mu2

p adj= P valor ajustado ==\> como pvalor= 0.0000 \< alfa=0.05 ==\> acepto Ho: mu4 \> mu3

p adj=P valor ajustado ==\> Como p valor = 0.9995 \> alfa = 0.05 ==\> Acepto Ho: mu5\> mu3

p adj= P valor ajustado ==\> como pvalor= 0.0000 \> alfa=0.05 ==\> Rechazo Ho: mu6 \< mu3

p adj= P valor ajustado ==\> como pvalor= 0.0000 \> alfa=0.05 ==\> Rechazo Ho: mu5 \< mu4

p adj= P valor ajustado ==\> como pvalor= 0.0000 \> alfa=0.05 ==\> Rechazo Ho: mu6\< mu4

p adj= P valor ajustado ==\> como pvalor= 0.0000 \> alfa=0.05 ==\> Rechazo Ho: mu6 \< mu5
...
```{r echo=TRUE}
plot(TukeyHSD(Anova, conf.level=0.95), las=2) # Anova es el nombre de la tabla de ANOVA que se hallo mas arriba
```
# SUPUESTOS DE ANOVA:
